...
namespace PlanningPoker.FunctionalTests.Tests.Hubs
{
    public class PlanningPokerHubTestBuilder
    {
        ...
        {
            return WithPlayer(serverId, Guid.NewGuid(), PlayerType.Observer, PlayerTag.None, Guid.NewGuid().ToString(), out player);
        }
        ...
        private PlanningPokerHubTestBuilder WithPlayer(Guid serverId, Guid recoveryId, PlayerType playerType, PlayerTag playerTag, string playerName, out PlayerViewModel player)
        {
            HubClient.Connect(serverId).GetAwaiter().GetResult();
            player = AwaitEventResult<PokerServerViewModel, PlayerViewModel>(
                () => HubClient.JoinServer(serverId, recoveryId, playerName, playerType, playerTag),
